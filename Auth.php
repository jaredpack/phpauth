...
class Auth
{
     ...
     */
    //@todo: => loginUser
    public function login(int $uid, string $password, int $remember = 0, string $captcha_response = ''): array
    {
        $return          = [];
        $return['error'] = true;
        $return['hash']  = '';
    ...
    public function register(string $password, string $repeatpassword, array $params = [], string $captcha_response = '', bool $use_email_activation = false): array
    {
        $addUser = $this->addUser($password, $params, $use_email_activation);
     ...
     * @return array
     */
    protected function addUser(string $password, array $params = [], bool $use_email_activation = false): array
    {
        $return['error'] = true;
        ...
        $query = "UPDATE {$this->config->table_users} SET password = ?, isactive = ? {$setParams} WHERE id = ?";
        $query_prepared = $this->dbh->prepare($query);

        $bindParams = array_values(array_merge([$password, $isactive], $params, [$uid]));
    ...
    {
        $query = "SELECT password, isactive FROM {$this->config->table_users} WHERE id = :id";
        $query_prepared = $this->dbh->prepare($query);
